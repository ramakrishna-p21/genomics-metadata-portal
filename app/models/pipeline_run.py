from sqlalchemy import ForeignKey, Integer, JSON, Text, TIMESTAMP, BigInteger, func
...
class PipelineRun(Base):
    ...
    seq_run_id: Mapped[str | None] = mapped_column(ForeignKey("sequencing_runs.seq_run_id"), nullable=True)
    pipeline_version_id: Mapped[str] = mapped_column(ForeignKey("pipeline_versions.pipeline_version_id"), nullable=False)
    ...
    parameter_set_json: Mapped[dict] = mapped_column(JSON, nullable=False, default=dict, server_default="{}")
...
class PipelineRunReference(Base):
    ...
    pipeline_run_reference_id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=True)
    pipeline_run_id: Mapped[str] = mapped_column(ForeignKey("pipeline_runs.pipeline_run_id"), nullable=False)
    reference_id: Mapped[str] = mapped_column(ForeignKey("reference_resources.reference_id"), nullable=False)
...
class PipelineRunTool(Base):
    ...
    pipeline_run_tool_id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=True)
    pipeline_run_id: Mapped[str] = mapped_column(ForeignKey("pipeline_runs.pipeline_run_id"), nullable=False)
