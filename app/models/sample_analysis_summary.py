from sqlalchemy import ForeignKey, Numeric, Text, TIMESTAMP, JSON, func
...
class SampleAnalysisSummary(Base):
    ...
    sample_id: Mapped[str] = mapped_column(ForeignKey("samples.sample_id"), nullable=False, unique=True)
    ...
    analysis_summary_json: Mapped[dict] = mapped_column(JSON, nullable=False, default=dict, server_default="{}")
    last_updated_at: Mapped[object] = mapped_column(TIMESTAMP(timezone=True), nullable=False, server_default=func.now())
