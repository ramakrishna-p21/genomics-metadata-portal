from sqlalchemy import BigInteger, Boolean, Date, ForeignKey, Text, TIMESTAMP, func
...
class SequencingRun(Base):
    ...
    paired_end: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True, server_default="true")
...
class SampleRunAssignment(Base):
    ...
    sample_run_assignment_id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=True)
    sample_id: Mapped[str] = mapped_column(ForeignKey("samples.sample_id"), nullable=False)
    seq_run_id: Mapped[str] = mapped_column(ForeignKey("sequencing_runs.seq_run_id"), nullable=False)
